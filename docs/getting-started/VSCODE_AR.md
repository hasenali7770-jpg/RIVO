...
## أسرع طريق: أمر واحد يسوّي كل شي

بعد ما تنزّل المشروع وتفتحه بـ VS Code:

**`Ctrl+Shift+P`** ← `Tasks: Run Task` ← **`★ التهيئة والتشغيل — أول مرّة`**
...
1. يفحص الأدوات ويقول لك بالضبط شنو ينقصك مع رابط تنزيله
2. يختار المسار تلقائياً — Docker إذا شغّال، وإلا PostgreSQL و Redis المحليّين
3. ينشئ `.env` بمفاتيح تشفير مولّدة عشوائياً وكلمة مرور مدير قوية
4. يسألك عن مفاتيح Mapbox (اضغط Enter لتخطّيها)
5. ينصّب الحزم، ينشئ قاعدة البيانات، يثبّت PostGIS
6. يطبّق الهجرات ويملأ محتوى العرض
7. يشغّل كل شي ويطبع لك الروابط وكلمة المرور

**آمن للتكرار:** ما يستبدل `.env` موجوداً بدون ما يسألك، وما يحذف أي قاعدة بيانات.

---

## أسرع أمر: وين وصلت؟

بأي لحظة، افتح الطرفية داخل VS Code (**``Ctrl+` ``**) وشغّل:

```bash
./infra/scripts/status.sh
```

أو من غير كتابة: **`Ctrl+Shift+B`**

يفحص كل شي — الأدوات، الإعدادات، قاعدة البيانات، الخدمات — ويطبع تقريراً يقول
لك بالضبط شنو جاهز وشنو الناقص وشنو الأمر اللي يصلّحه. **ما يطبع أي مفتاح ولا
كلمة مرور**، فتقدر تنسخه كما هو وترسله لأي أحد.

> على ويندوز بدون WSL، شغّله من طرفية Git Bash (تجي مع Git for Windows):
> اضغط السهم جنب **`+`** بلوحة Terminal ← **Git Bash**.
...
| المهمّة | تسوي شنو |
|---|---|
| `★ التهيئة والتشغيل — أول مرّة` | يجهّز كل شي من الصفر ويشغّله |
| `الحالة: وين وصلت؟` | تقرير شامل بشنو جاهز وشنو ناقص (`Ctrl+Shift+B`) |
...
**قبل أي شي:** شغّل `./infra/scripts/status.sh` — يحدّد المشكلة ويقول الأمر اللي يصلّحها.
